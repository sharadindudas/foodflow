...
const Checkout = () => {
  ...
  const totalPrice = cartItems.reduce((prev, curr) => prev + (curr?.price / 100 || curr?.defaultPrice / 100) * curr?.quantity, 0)?.toFixed(2);
            ...
            {cartItems?.map((item) => (
              <div
                ...
                <div className="md:w-auto w-3/5">
                  ...
                  {item?.price ? (
                    <span className="rupee text-color-9 text-sm font-ProximaNovaMed">{item?.price / 100}</span>
                  ) : (
                    <span className="rupee text-color-9 text-sm font-ProximaNovaMed">{item?.defaultPrice / 100}</span>
                  )}
                  {item?.description && (
                    <p className="text-color-10 mt-3 tracking-tight font-ProximaNovaThin text-sm md:w-3/4">{item?.description}</p>
                  )}
                </div>
              ...
              {resInfo?.deliveryfee ? (
                <div>
                  <span className="rupee font-ProximaNovaSemiBold">
                    {parseInt(totalPrice + resInfo?.deliveryfee)} {`(${String(parseInt(totalPrice))} + ${String(resInfo?.deliveryfee)})`}
                  </span>
                </div>
              ) : (
                <div>
