...
const RestaurantSection = ({ AllRestaurants, userLocation, FilteredRestaurants, setFilteredRestaurants }) => {
  ...
  const handleOffers = () => {
    setFilteredRestaurants(
      AllRestaurants.filter((res) => res?.info?.aggregatedDiscountInfoV3?.header || res?.info?.aggregatedDiscountInfoV3?.subHeader)
    );
  };
  ...
  const handlePriceRange300to600 = () => {
    const MinPrice = "300",
      MaxPrice = "600";
    setFilteredRestaurants(
      AllRestaurants.filter((res) => res?.info?.costForTwo?.slice(1, 4) >= MinPrice && res?.info?.costForTwo?.slice(1, 4) <= MaxPrice)
    );
  };
        ...
        <>
          <section id="restaurants">
            <h2 className="font-GrotBlack text-2xl pb-5 pt-5 2xl:text-start text-center sm:px-0 px-2">
              Restaurants with online food delivery in {userLocation?.city ? userLocation?.city : "Bangalore"}
            </h2>
