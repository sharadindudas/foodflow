...
const RestaurantCategory = ({ title, itemCards, ShowItem, handleShowItem, ResInfoData }) => {
  ...
  return (
    <>
      ...
      <div
        ...
        <div className="text-xl text-color-9">{ShowItem ? <IoIosArrowUp /> : <IoIosArrowDown />}</div>
      </div>
              ...
              <div
                ...
                <div className="md:w-auto w-3/5">
                  ...
                  {item?.card?.info?.price ? (
                    <span className="rupee text-color-9 text-sm font-ProximaNovaMed">{item?.card?.info?.price / 100}</span>
                  ) : (
                    <span className="rupee text-color-9 text-sm font-ProximaNovaMed">{item?.card?.info?.defaultPrice / 100}</span>
                  )}
                  {item?.card?.info?.description && (
                    <p className="text-color-10 mt-3 tracking-tight font-ProximaNovaThin text-sm md:w-3/4">{item?.card?.info?.description}</p>
                  )}
                </div>
